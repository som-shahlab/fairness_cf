    ...
    def __init__(
        ...
        if num_hidden == 0:
            output_layer = LinearLayerWrapper(
                in_features, output_dim, sparse=sparse, sparse_mode=sparse_mode
            )
...
class HiddenLinearLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.linear = LinearLayerWrapper(
            in_features, out_features, sparse=sparse, sparse_mode=sparse_mode
        )
...
class ResidualBlock(torch.nn.Module):
    ...
    def __init__(
        self, hidden_dim, drop_prob=0.0, normalize=False, activation=F.leaky_relu
    ):
        super().__init__()
