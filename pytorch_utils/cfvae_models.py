...
class CFVAEModel(TorchModel):
    ...
    def init_encoder(self, config_dict):
        ...
        """
        hidden_dim_list = [
            config_dict["latent_dim"] * (2 ** (i + 1))
            for i in reversed(range(config_dict["num_hidden"]))
        ]
        encoder = FeedforwardNet(
            in_features=config_dict["input_dim"] + config_dict["num_groups"],
        ...
        return VAEEncoder(
            encoder=encoder, reparameterization_layer=reparameterization_layer
        )
    ...
    def init_decoder(self, config_dict):
        ...
        """
        hidden_dim_list = [
            (config_dict["latent_dim"] + config_dict["group_embed_dim"])
            * (2 ** (i + 1))
            for i in range(config_dict["num_hidden"])
        ]
        decoder = FeedforwardNet(
            in_features=config_dict["latent_dim"] + config_dict["group_embed_dim"],
            hidden_dim_list=hidden_dim_list,
    ...
    def init_classifier(self, config_dict):
        ...
        decoder = FeedforwardNet(
            in_features=config_dict["latent_dim"] + config_dict["group_embed_dim"],
            hidden_dim_list=config_dict["num_hidden_classifier"]
            * [config_dict["hidden_dim_classifier"]],
    ...
    def init_model(self):
        ...
        """
        return CFVAE(
            encoder=self.encoder, decoder=self.decoder, classifier=self.classifier
        )
    ...
    def init_final_classifier_optim(self):
        ...
        return torch.optim.Adam(
            ...
            lr=self.config_dict["lr_final_classifier"],
        )
    ...
    def compute_kernel(x, y):
        ...
        kernel_input = (tiled_x - tiled_y).pow(2).mean(2) / float(
            dim
        )  # scale-invariant gamma
    ...
    def init_performance_dict(
        self, metrics=["auc", "auprc", "brier"], phases=["train", "val"]
    ):
        """
        ...
        for epoch in range(self.config_dict["num_epochs"]):
            ...
            for phase in ["train", "val"]:
                ...
                for the_data in loaders[phase]:
                    ...
                    combined_inputs = scipy.sparse.hstack(
                        (inputs, group_binarizer.transform(group.cpu().numpy())),
                        format="csr",
                    )
                    ...
                    # Classification
                    batch_loss_dict["classification"] = self.criterion_classification(
                        y_outputs, labels
                    )
                    output_dict = self.update_output_dict(
                        output_dict, y_outputs, labels
                    )

                    # Group MMD (see Louizos 2016: http://arxiv.org/abs/1511.00830)
                    batch_loss_dict["mmd_group"] = self.compute_mmd_group(
                        z, prior_samples, group
                    )

                    # Aggregate the losses
                    batch_loss_dict["loss"] = (
                        (
                            self.config_dict["lambda_reconstruction"]
                            * batch_loss_dict["reconstruction"]
                        )
                        + (self.config_dict["lambda_mmd"] * batch_loss_dict["mmd"])
                        + (self.config_dict["lambda_kl"] * batch_loss_dict["kl"])
                        + (
                            self.config_dict["lambda_classification"]
                            * batch_loss_dict["classification"]
                        )
                        + (
                            self.config_dict["lambda_mmd_group"]
                            * batch_loss_dict["mmd_group"]
                        )
                    )
                    # ELBO
    ...
    def update_output_dict_sampling(
        self, output_dict_sampling, sampling_dict, outputs_factual, outputs_cf
    ):
        for key in sampling_dict.keys():
            if key in output_dict_sampling.keys():
                output_dict_sampling[key] = torch.cat(
                    (output_dict_sampling[key], sampling_dict[key].detach().cpu()),
                    dim=0,
                )
        ...
        output_dict_sampling["pred_prob_factual"] = torch.cat(
            (
                output_dict_sampling["pred_prob_factual"],
                pred_prob_factual[:, 1].detach().cpu(),
            )
        )
        output_dict_sampling["pred_prob_cf"] = torch.cat(
            (output_dict_sampling["pred_prob_cf"], pred_prob_cf[:, 1].detach().cpu())
        )
        output_dict_sampling["output_factual"] = torch.cat(
            (
                output_dict_sampling["output_factual"],
                outputs_factual[:, 1].detach().cpu(),
            )
        )
        output_dict_sampling["output_cf"] = torch.cat(
            (output_dict_sampling["output_cf"], outputs_cf[:, 1].detach().cpu())
        )
    ...
    def compute_sampling_dict(self, z, labels, group, num_samples=1):
        ...
        """
        sampling_dict = {
            key: [] for key in ["y", "y_cf", "z", "group", "group_cf", "y_mask"]
        }
        ...
        for i in range(num_samples):
            for group_cf in range(self.config_dict["num_groups"]):
                ...
                temp_dict["y_cf"] = self.sample_labels(
                    self.model.classifier(
                        temp_dict["z"].detach(), temp_dict["group_cf"].detach()
                    )
                )
                temp_dict["y_mask"] = (
                    temp_dict["y"] == temp_dict["y_cf"]
                )  # elements with the same label of y
                group_mask = (
                    temp_dict["group"] != temp_dict["group_cf"]
                )  # elements that are counterfactua
    ...
    def compute_sampling_dict_weighted(self, z, labels, group):
        ...
        sampling_dict = {
            key: []
            for key in ["y", "y_cf", "z", "group", "group_cf", "y_mask", "prob_y_cf"]
        }
    ...
    def clp_entropy_loss(
        self, outputs_factual, outputs_cf, weights=None, reduction="sum"
    ):
        if weights is None:
            return F.binary_cross_entropy_with_logits(
                outputs_factual, F.softmax(outputs_cf, dim=1), reduction=reduction
            )
        else:
            result = F.binary_cross_entropy_with_logits(
                outputs_factual, F.softmax(outputs_cf, dim=1), reduction="none"
            )
            return (result * weights.reshape(-1, 1)).sum()
            ...
            for phase in ["train", "val"]:
                ...
                for the_data in loaders[phase]:
                    ...
                    if self.config_dict["weighted"]:
                        sampling_dict = self.compute_sampling_dict_weighted(
                            z, labels, group
                        )
                    else:
                    ...
                    y_outputs_factual = self.final_classifier(
                        sampling_dict["z"], sampling_dict["group"]
                    )  # the factual y corresponding to the cf y's
                    y_outputs_cf = self.final_classifier(
                        sampling_dict["z"], sampling_dict["group_cf"]
                    )

                    batch_loss_dict[
                        "classification_cf"
                    ] = self.criterion_classification(
                        y_outputs_cf, sampling_dict["y_cf"]
                    )

                    ## Compute the CLP loss
                    # Get the weights
                    weights = (
                        sampling_dict["prob_y_cf"][sampling_dict["y_mask"]]
                        if self.config_dict["weighted"]
                        else None
                    )
                    ...
                    y_outputs_cf = (
                        y_outputs_cf
                        if self.config_dict["cf_gradients"]
                        else y_outputs_cf.detach()
                    )
                    ...
                    batch_loss_dict["loss"] = (
                        batch_loss_dict["classification"]
                        + self.config_dict["lambda_final_classifier_cf"]
                        * batch_loss_dict["classification_cf"]
                        + (
                            self.config_dict["lambda_clp"]
                            * batch_loss_dict["clp"]
                            / num_mask_batch
                        )
                        + (
                            self.config_dict["lambda_clp_entropy"]
                            * batch_loss_dict["clp_entropy"]
                            / num_mask_batch
                        )
                    )
                    ...
                    output_dict_factual = self.update_output_dict(
                        output_dict_factual, y_outputs_factual, sampling_dict["y"]
                    )
                    output_dict_cf = self.update_output_dict(
                        output_dict_cf, y_outputs_cf, sampling_dict["y_cf"]
                    )
                    output_dict_sampling = self.update_output_dict_sampling(
                        output_dict_sampling,
                        sampling_dict,
                        y_outputs_factual,
                        y_outputs_cf,
                    )
                ...
                # Compute Losses
                scaling_dict = {
                    key: i if key not in ["clp", "clp_entropy"] else int(num_mask.cpu())
                    for key in running_loss_dict.keys()
                }
                epoch_loss_dict = {
                    key: running_loss_dict[key] / scaling_dict[key]
                    for key in running_loss_dict.keys()
                }
                # Update the loss dict
                loss_dict[phase] = self.update_metric_dict(
    ...
    def predict_final_classifier_CLP(
        self, data_dict, label_dict, group_dict, phases=["test"]
    ):
        """
        Train the final classifier
        """
        ...
        with torch.no_grad():
            ...
            for phase in phases:
                ...
                for the_data in loaders[phase]:
                    ...
                    sampling_dict = self.compute_sampling_dict(
                        z,
                        labels,
                        group,
                        num_samples=self.config_dict["num_samples_eval"],
                    )
                    ...
                    batch_loss_dict["clp"] = self.clp_loss(
                        y_outputs_factual[sampling_dict["y_mask"], :],
                        y_outputs_cf[sampling_dict["y_mask"], :],
                        reduction="sum",
                    )
                    batch_loss_dict["clp_entropy"] = self.clp_entropy_loss(
                        y_outputs_factual[sampling_dict["y_mask"], :],
                ...
                performance_dict_cf[phase] = self.update_metric_dict(
                    performance_dict_cf[phase], epoch_statistics_cf
                )
    ...
    def predict_final_classifier(
        ...
        group_binarizer = self.init_binarizer(group_dict)
        loss_dict = self.init_loss_dict(
            metrics=["loss", "classification"], phases=phases
        )
        performance_dict = self.init_performance_dict(phases=phases)
        performance_dict_cf = self.init_performance_dict(phases=phases)
