...
class model_CLP(TorchModel):
    def init_loaders(self, data_dict, data_dict_cf, label_dict):
        ...
        dataset_dict = {
            key: TensorDataset(
                torch.FloatTensor(data_dict[key]),
        ...
        loaders_dict = {
            key: DataLoader(
                dataset_dict[key], batch_size=self.config_dict["batch_size"]
            )
            for key in data_dict.keys()
        }
        return loaders_dict
    ...
    def init_loss_dict(
        self, metrics=["loss", "classification", "clp"], phases=["train", "val"]
    ):
...
class model_CLP_conditional(TorchModel):
    ...
    def init_loss_dict(
        self,
        metrics=["loss", "classification", "clp", "classification_cf"],
        phases=["train", "val"],
    ):
        return super().init_loss_dict(metrics=metrics, phases=phases)

    def train(self, data_dict, data_dict_cf, label_dict, label_dict_cf):
        loaders = self.init_loaders(
            data_dict=data_dict,
            data_dict_cf=data_dict_cf,
            label_dict=label_dict,
            label_dict_cf=label_dict_cf,
        )
        best_performance = 1e18
        loss_dict = self.init_loss_dict()
        performance_dict = self.init_performance_dict()
        ...
        for epoch in range(self.config_dict["num_epochs"]):
            ...
            for phase in ["train", "val"]:
                ...
                for the_data in loaders[phase]:
                    i += 1
                    batch_loss_dict = {}
                    inputs, inputs_cf, labels, labels_cf = self.transform_batch(
                        the_data
                    )
                    ...
                    output_dict_cf = self.update_output_dict(
                        output_dict_cf, outputs_cf, labels_cf
                    )

                    ## Classification losses
                    batch_loss_dict["classification"] = self.criterion(outputs, labels)
                    batch_loss_dict["classification_cf"] = self.criterion(
                        outputs_cf, labels_cf
                    )
                    ...
                    batch_loss_dict["clp"] = (
                        (outputs[mask] - outputs_cf[mask]) ** 2
                    ).mean()

                    batch_loss_dict["loss"] = (
                        batch_loss_dict["classification"]
                        + self.config_dict["lambda_cls_cf"]
