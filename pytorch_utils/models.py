...
class TorchModel:
    ...
    def update_output_dict(self, output_dict, outputs, labels):
        ...
        output_dict["outputs"] = torch.cat(
            (output_dict["outputs"], outputs.detach().cpu())
        )
        output_dict["pred_probs"] = torch.cat(
            (output_dict["pred_probs"], pred_probs.detach().cpu())
        )
        output_dict["labels"] = torch.cat(
            (output_dict["labels"], labels.detach().cpu())
        )
